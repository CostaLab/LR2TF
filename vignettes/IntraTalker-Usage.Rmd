---
title: "IntraTalker-Usage"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{IntraTalker-Usage}
...
IntraTalker depends on two Bioconductor packages, `ComplexHeatmap` (for the
transcription factor activity heatmaps) and `scran` (for marker detection in the
differential analysis). These are not available from CRAN and are best installed
with `BiocManager` before the package itself:
&nbsp;
...
All other dependencies are on CRAN and are installed automatically. The package
itself can then be installed directly from github:
&nbsp;

```{r, include = TRUE, eval = FALSE}
library(devtools)
install_github("CostaLab/IntraTalker")

library(remotes)
install_github("CostaLab/IntraTalker", build_vignettes = TRUE)
```
&nbsp;
...
data(bone_marrow_stromal_cell_example, package = "IntraTalker")
...
regulon <- read.csv(system.file("regulons", "human_dorothea_reg.csv", package = "IntraTalker"))
...
## Using the IntraTalker package
Now, that we have a transcription factor activity matrix, we can continue the analysis with the IntraTalker package. In this case we will use our test data for this example. First of all, it is necessary to define the following parameters in form of a list:
...
```{r, include = TRUE, eval = FALSE}
results <- IntraTalker::IntraTalker_analysis(
...
ctr_input <- IntraTalker::combine_LR_and_TF_complexes(
  tf_table = results@CTR_input_condition[["control"]],
  LR_prediction = table_ctr,
  out_path = parameters$out_path,
  condition = "control"
)
exp_input <- IntraTalker::combine_LR_and_TF_complexes(
